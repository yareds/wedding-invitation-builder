...
import { Palette, Heart, Image as ImageIcon, Music, MapPin, Plus, Trash2, Check, ShoppingBag, Monitor, Smartphone, Upload } from 'lucide-react';
...
export const WeddingBuilder: React.FC<WeddingBuilderProps> = ({
  ...
  return (
    <aside className="w-full lg:w-[450px] xl:w-[480px] bg-white border-r border-[#D4849A]/30 flex flex-col h-full shadow-lg overflow-hidden">
      {/* Top Builder Branding Header */}
      <div className="p-4 bg-[#3B0B1F] text-[#FDF0F3] border-b border-[#C8A84B]/40 flex items-center justify-between">
        <div>
          <span className="font-body text-[10px] uppercase tracking-[0.25em] text-[#D4849A] block font-semibold">
            Real-Time Customizer
          </span>
          <h1 className="font-serif-heading text-lg font-normal text-[#FDF0F3]">
            Wedding Invitation Builder
          </h1>
        </div>

        {/* View Mode Toggle */}
        <div className="flex items-center gap-1 bg-[#2D0817] p-1 rounded-full border border-[#C8A84B]/40">
          <button
            onClick={() => onToggleDeviceMode('desktop')}
            className={`p-1.5 rounded-full transition-colors cursor-pointer ${
              deviceMode === 'desktop' ? 'bg-[#C8A84B] text-[#3B0B1F]' : 'text-[#FDF0F3]/70 hover:text-[#FDF0F3]'
            }`}
            title="Desktop View"
          >
          ...
          <button
            ...
            className={`p-1.5 rounded-full transition-colors cursor-pointer ${
              deviceMode === 'mobile' ? 'bg-[#C8A84B] text-[#3B0B1F]' : 'text-[#FDF0F3]/70 hover:text-[#FDF0F3]'
            }`}
            title="Mobile View"
          >
      ...
      <div className="flex items-center justify-between border-b border-[#D4849A]/30 bg-[#FDF0F3]/60 px-2 py-2 overflow-x-auto no-scrollbar">
        {steps.map((step) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-body font-semibold transition-all whitespace-nowrap cursor-pointer ${
                isActive
                  ? 'bg-[#3B0B1F] text-[#FDF0F3] shadow-md'
                  : 'text-[#3B0B1F]/70 hover:text-[#3B0B1F] hover:bg-[#D4849A]/20'
              }`}
            >
      ...
      <div className="p-4 bg-[#FDF0F3] border-t border-[#D4849A]/30 space-y-2">
        <div className="flex items-center justify-between text-xs text-[#3B0B1F]">
          <span>Package Price (Fixed):</span>
          <span className="font-serif-heading font-bold text-base text-[#3B0B1F]">30,000 ETB</span>
        </div>
