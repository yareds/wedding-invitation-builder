...
export type BotanicalFramePosition = 'top-left' | 'top-right' | 'bottom-left' | 'bottom-right' | 'banner-top' | 'banner-bottom';
...
export function getBotanicalFrameHtml(
  ...
  const getThemeColors = () => {
    switch (themeId) {
      case 'emerald':
        return { gold: '#D4AF37', leaf: '#2D6A4F', petal: '#52B788', accent: '#E9C46A' };
      case 'midnight':
        return { gold: '#E8C96A', leaf: '#2E1D6B', petal: '#7C3AED', accent: '#DDD6FE' };
      case 'rosegarden':
        return { gold: '#FCD34D', leaf: '#9A3412', petal: '#C2410C', accent: '#FCA5A5' };
      case 'goldluxury':
        return { gold: '#D4AF37', leaf: '#2D2D2D', petal: '#4A4A4A', accent: '#F3E5AB' };
      case 'classicivory':
        return { gold: '#B8860B', leaf: '#423E3C', petal: '#615C59', accent: '#E6D7C3' };
      case 'bordeaux':
      default:
        return { gold: '#C8A84B', leaf: '#721121', petal: '#E5989B', accent: '#F3C969' };
    }
  };
  ...
  if (position === 'banner-top' || position === 'banner-bottom') {
    ...
    if (themeId === 'emerald') {
      svgContent = `
        <svg viewBox="0 0 800 60" fill="none" class="w-full max-w-4xl h-auto opacity-90 ${isBottom ? 'rotate-180' : ''}">
          <path d="M50 30 Q 200 15, 400 30 Q 600 45, 750 30" stroke="${goldColor}" stroke-width="1.5" stroke-linecap="round" />
          ...
          <g transform="translate(400, 30)">
            ...
            ${[0, 60, 120, 180, 240, 300].map(angle => `<path d="M0 0 C -6 -14, 6 -14, 0 0" fill="${petalColor}" stroke="${goldColor}" stroke-width="0.8" transform="rotate(${angle})" />`).join('')}
      ...
      svgContent = `
        <svg viewBox="0 0 800 60" fill="none" class="w-full max-w-4xl h-auto opacity-90 ${isBottom ? 'rotate-180' : ''}">
          <path d="M50 30 L 350 30 M 450 30 L 750 30" stroke="${goldColor}" stroke-width="1" stroke-dasharray="4 4" />
    ...
    } else if (themeId === 'rosegarden') {
      svgContent = `
        <svg viewBox="0 0 800 60" fill="none" class="w-full max-w-4xl h-auto opacity-85 ${isBottom ? 'rotate-180' : ''}">
          <path d="M 60 40 Q 200 10, 400 15 Q 600 10, 740 40" stroke="${goldColor}" stroke-width="1.5" />
          <path d="M 100 45 Q 250 20, 400 25 Q 550 20, 700 45" stroke="${goldColor}" stroke-width="0.8" stroke-dasharray="3 3" />
          <g transform="translate(400, 25)">
            ${[0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330].map(ang => `<line x1="0" y1="0" x2="16" y2="0" stroke="${goldColor}" stroke-width="1" transform="rotate(${ang})" />`).join('')}
            <circle cx="0" cy="0" r="6" fill="${leafColor}" stroke="${goldColor}" stroke-width="1" />
          </g>
        </svg>`;
    } else if (themeId === 'goldluxury') {
      svgContent = `
        <svg viewBox="0 0 800 60" fill="none" class="w-full max-w-4xl h-auto opacity-90 ${isBottom ? 'rotate-180' : ''}">
          <path d="M 50 30 L 370 30 M 430 30 L 750 30" stroke="${goldColor}" stroke-width="1.5" />
          <path d="M 70 25 L 360 25 M 440 25 L 730 25" stroke="${goldColor}" stroke-width="0.8" />
          <g transform="translate(400, 30)">
    ...
    } else if (themeId === 'classicivory') {
      svgContent = `
        <svg viewBox="0 0 800 60" fill="none" class="w-full max-w-4xl h-auto opacity-80 ${isBottom ? 'rotate-180' : ''}">
          <line x1="80" y1="30" x2="380" y2="30" stroke="${goldColor}" stroke-width="1" />
          <line x1="420" y1="30" x2="720" y2="30" stroke="${goldColor}" stroke-width="1" />
          <circle cx="400" cy="30" r="5" stroke="${goldColor}" stroke-width="1" fill="none" />
      ...
      svgContent = `
        <svg viewBox="0 0 800 60" fill="none" class="w-full max-w-4xl h-auto opacity-85 ${isBottom ? 'rotate-180' : ''}">
          ...
          <g transform="translate(400, 24) scale(0.9)">
            <path d="M-15,10 L-12,-5 L-5,2 L0,-10 L5,2 L12,-5 L15,10 Z" fill="${goldColor}" stroke="${leafColor}" stroke-width="0.8" />
    ...
    return `<div class="w-full overflow-hidden pointer-events-none flex justify-center py-2 ${className}">${svgContent}</div>`;
  }

  let cornerSvg = '';
  if (themeId === 'emerald') {
    cornerSvg = `
      <svg width="150" height="150" viewBox="0 0 150 150" fill="none" class="w-24 h-24 sm:w-36 sm:h-36">
        ...
        <g transform="translate(50, 50)">
          <circle cx="0" cy="0" r="5" fill="${accentColor}" />
          ${[0, 60, 120, 180, 240, 300].map(angle => `<path d="M0 0 C -7 -15, 7 -15, 0 0" fill="${petalColor}" stroke="${leafColor}" stroke-width="0.8" transform="rotate(${angle})" />`).join('')}
    ...
    cornerSvg = `
      <svg width="150" height="150" viewBox="0 0 150 150" fill="none" class="w-24 h-24 sm:w-36 sm:h-36">
        <path d="M 12 138 C 12 60, 60 12, 138 12" stroke="${goldColor}" stroke-width="1.2" stroke-dasharray="3 3" />
    ...
    cornerSvg = `
      <svg width="150" height="150" viewBox="0 0 150 150" fill="none" class="w-24 h-24 sm:w-36 sm:h-36">
        <path d="M 10 140 L 10 10 L 140 10" stroke="${goldColor}" stroke-width="2" />
        <path d="M 18 140 L 18 18 L 140 18" stroke="${leafColor}" stroke-width="0.8" />
        <g transform="translate(18, 18)">
          ${[0, 18, 36, 54, 72, 90].map(ang => `<line x1="0" y1="0" x2="30" y2="0" stroke="${goldColor}" stroke-width="1" transform="rotate(${ang})" />`).join('')}
    ...
    cornerSvg = `
      <svg width="150" height="150" viewBox="0 0 150 150" fill="none" class="w-24 h-24 sm:w-36 sm:h-36">
        <path d="M 10 140 L 10 35 L 35 10 L 140 10" stroke="${goldColor}" stroke-width="2" />
        <path d="M 20 130 L 20 40 L 40 20 L 130 20" stroke="${goldColor}" stroke-width="0.8" />
    ...
    cornerSvg = `
      <svg width="150" height="150" viewBox="0 0 150 150" fill="none" class="w-24 h-24 sm:w-36 sm:h-36">
        <path d="M 12 140 L 12 12 L 140 12" stroke="${goldColor}" stroke-width="1" />
        <path d="M 20 140 L 20 20 L 140 20" stroke="${goldColor}" stroke-width="0.5" />
    ...
    cornerSvg = `
      <svg width="150" height="150" viewBox="0 0 150 150" fill="none" class="w-24 h-24 sm:w-36 sm:h-36">
        ...
        <g transform="translate(32, 32) scale(0.9)">
          <path d="M-12,8 L-10,-4 L-4,1 L0,-8 L4,1 L10,-4 L12,8 Z" fill="${goldColor}" stroke="${leafColor}" stroke-width="0.8" />
...
export const BotanicalFrame: React.FC<BotanicalFrameProps> = ({
  ...
  const html = getBotanicalFrameHtml(position, themeId, className);
